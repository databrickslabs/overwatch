...
[Getting Started - Run Via Notebook]({{%relref "DeployOverwatch/RunningOverwatch/NotebookLegacy"%}}#jump-start-notebooks) or
[Getting Started - Run Via Main Class]({{%relref "DeployOverwatch/RunningOverwatch/JARLegacy"%}}#via-main-class) section to see how it's can be used
...
can be found in the [Azure Environment Setup]({{%relref "DeployOverwatch/CloudInfra/Azure.md"%}}) page. An example notebook
...
**overwatchScope**|N|all|Option\[Seq\[String\]\]|0.5.x|List of [modules]({{%relref "DataEngineer/Modules.md"%}}) in scope for the run. It's important to note that there are many co-dependencies. When choosing a module, be sure to also enable it's requisites. If not value provided, all modules will execute.
...
### TokenSecret
Overwatch must have permission to perform its functions; these are further discussed in [AdvancedTopics]({{%relref "DataEngineer/AdvancedTopics.md"%}}).
...
and entire [database migration]({{%relref "DataEngineer/advancedtopics.md"%}}). Overwatch can perform destructive tasks within its own database and
