  ...
  protected def buildNotebookCommandsFact(
    ...
    if (!auditDF_base.columns.contains("executionTime")) {
      ...
      throw new VerboseAuditLoggingDisabled(s"Verbose Audit Logging is not enabled in the workspace. To get the data in notebookCommands_gold it is necessary you " +
        s"enable verbose audit logging in the workspace. Also you can exclude scope notebookCommands if you don't want to get notebookCommands_gold table to be populated")
    }else{
