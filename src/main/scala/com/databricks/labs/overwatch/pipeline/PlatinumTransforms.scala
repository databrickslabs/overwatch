...
//   if (spark.catalog.tableExists(s"${platinum_table}")){
//     print("Plantinum Table Exists. Merge Operation in Progress!!")
//     val deltaTableName = s"${platinum_table}"
//     val deltaTable = DeltaTable.forName(deltaTableName)
//     val sourceDF = cluster_cost_details
//
//     deltaTable.as("target")
//       .merge(
//         sourceDF.as("source"),
//         """
//         target.cluster_id = source.cluster_id AND
//         target.cluster_category = source.cluster_category AND
//         target.date = source.date
//         """
//       )
//       .whenMatched()
//       .updateExpr(Map(
//         "total_dbus" -> "source.total_dbus",
//         "`total_dbu_cost_USD`" -> "source.`total_dbu_cost_USD`",
//         "`total_compute_cost_USD`" -> "source.`total_compute_cost_USD`"
//       ))
//       .whenNotMatched()
//       .insertAll()
//       .execute()
//   }else{
//     cluster_cost_details.write.saveAsTable(s"${platinum_table}")
//   }
