...
trait SilverTransforms extends SparkSessionWrapper {
  ...
  def buildClusterStateDetail(
    ...
    val clusterEventsBaseline = clusterEventsDF
      ...
      .withColumn("previousState",lag($"state", 1, null).over(stateUnboundW))
      .withColumn("isRunning",when(col("previousState") === "false" && col("state") === "EXPANDED_DISK",lit(false)).otherwise('isRunning))
      .drop("previousState")
      .withColumn(
        "current_num_workers",
        coalesce(
