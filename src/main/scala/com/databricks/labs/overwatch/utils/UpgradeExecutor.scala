...
class UpgradeExecutor(val etldbname: String, val fromVersion: Int, val toVersion: Int, val executeSingleStep: Int = -1) {
  ...
  val workspace = Helpers.getWorkspaceByDatabase(etldbname)
    ...
    versionMap.foreach(
      {
        pair =>
          ...
          if (is_overlapping(versionPair(0), versionPair(1), fromVersion, toVersion)) {
            upgradeChain += pair._2.getDeclaredConstructor(classOf[Workspace]).newInstance(workspace)
          }
      }
    )
    ...
    stepMap.foreach({
      step =>
        if (executeSingleStep <= step._1) {
          upgradeChain += versionMap.get(step._2).get.getDeclaredConstructor(classOf[Workspace], classOf[Int], classOf[Int]).newInstance(workspace, executeSingleStep.asInstanceOf[AnyRef], executeSingleStep.asInstanceOf[AnyRef])
        }
    })
